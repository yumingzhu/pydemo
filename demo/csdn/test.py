...
if (os.path.exists(logfile)):
    ...
    if (setting["fileSize"] < fileSize and setting["count"] < 3):
        file = io.open(logfile, "r", encoding='utf-8')
        file.seek(setting["fileSize"], 0)
        flag = False
        error = "";
        count = 0
        for line in file.readlines():
            # 出现异常就发邮件
            if (line.find("ERROR c.g.subentry.webapp.config.GlobalExceptionHandler") >= 0):
                flag = True
            if (flag and count < 3):
                error = error + line
                if (count == 2):
